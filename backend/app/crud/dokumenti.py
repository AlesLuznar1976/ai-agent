"""CRUD operacije za Dokumenti tabelo"""
...
def create_dokument(
    ...
    """Ustvari nov dokument zapis"""
    # Preveri verzijo - ali obstaja dokument z istim imenom
    existing = (
        db.query(DBDokument)
        .filter(DBDokument.projekt_id == projekt_id)
        .filter(DBDokument.naziv_datoteke == naziv_datoteke)
        .order_by(desc(DBDokument.verzija))
        .first()
    )
    verzija = (existing.verzija + 1) if existing else 1

    db_dokument = DBDokument(
        projekt_id=projekt_id,
        tip=tip,
        naziv_datoteke=naziv_datoteke,
        verzija=verzija,
        pot_do_datoteke=pot_do_datoteke,
        datum_nalozeno=datetime.now(),
        nalozil_uporabnik=nalozil_uporabnik,
    )
    db.add(db_dokument)
    db.commit()
